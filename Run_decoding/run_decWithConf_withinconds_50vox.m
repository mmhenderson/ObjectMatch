...
% Use trials from one condition at a time, 50 voxels
...
anovatypestr = 'matchTarg_crossval_balanceCorr';
...
%             correctinds = trialData(vv).predlabelsTBT(:,9)==1;
            ...
            if vv==1 && cc==1
                ...
                alldisteuc = nan(nVOIs,nCond,length(correctlabs),2);
                allpredlabs = nan(nVOIs,nCond,length(correctlabs),1);
                allcorrlabs = nan(length(correctlabs),nCond);
                allmatchlabs = nan(length(matchlabs),nCond);
            end
            
            allcorrlabs(:,cc) = correctlabs;
            allmatchlabs(:,cc) = matchlabs;
            ...
            [acc,dprime,predLabs,distEuc,failedInds] = my_classifier_normEucDist_balance2(dat,reallabs,correctlabs,runlabs,nBalanceIter1,nVox2Use_now,pTable(:,unruns),resamp);
            ...
            allpredlabs(vv,cc,:) = predLabs;
            alldisteuc(vv,cc,:,:) = squeeze(mean(distEuc,2));
            ...
            parfor ii=1:nShuffIter
                ...
                [acc,dprime,predLabs,distEuc,failedInds] = my_classifier_eucDist_balance2(dat,randlabs,correctlabs,runlabs,nBalanceIter2,nVox2Use_now,pTable(:,unruns),resamp);

                if sum(failedInds)>0
                    error('    %d trials in test set were not classified due to un-balanceable training set\n',sum(failedInds));               
                end
                
                allaccs_shuffDataLabs(vv,cc,ii) = acc;
                allD_shuffDataLabs(vv,cc,ii) = dprime;
            ...
            alldisteuc_shuffDataLabs(vv,cc).distEucRand = distEucRand;
