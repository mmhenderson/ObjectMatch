% Run decoding analysis for object target status - focusing on the IRRELEVANT
% feature dimension
% This is for a control analysis where we want to remove any trials that
% have high similarity between previous and current image. These will be
% matches in both orientation (view) and category. (_noOrCatMatch is suffix
% for the file that is saved)
% Use all voxels, within each condition separately


%% define subjects and flags for what to do
clear

% change this to your main dir
...
typestr = 'classify_targetIrr_withinCond_noOrCatMatch';
...
anovatypestr = 'matchTarg_crossval';
...
for ss=1:nSubj
    ...
    for vv=1:length(VOIs)
        ...
        for cc=1:nCond
            ...
            indsuse = correctinds & ~firstinds & thistask & ~orCatMatch;
            ...
            allmatchlabels = [IDmatchlabels,ORmatchlabels];
            
            reallabs = allmatchlabels(:,(3-cc));
            ...
            distEachRand = zeros(length(reallabs),2,nShuffIter);
