...
typestr1='classify_target_withinCond_noOrCatMatch';
typestr2='classify_targetIrr_withinCond_noOrCatMatch';
...
%% check single subject significance 

isSigAcc_SS = zeros(nVOIs,2,2,2);
isSigD_SS = zeros(nVOIs,2,2,2);

pAcc_SS = zeros(nVOIs,2,2,2);
pD_SS = zeros(nVOIs,2,2,2);

for ii=1:2
    for jj=1:2
        for ss=1:10

            pAcc_SS(:,ii,jj,ss) = 2*min([mean(squeeze(realAccs(:,ii,jj,ss))<squeeze(nullAccs(:,ii,jj,ss,:)),2), ...
                mean(squeeze(realAccs(:,ii,jj,ss))>squeeze(nullAccs(:,ii,jj,ss,:)),2)],[],2);
            [~,isSigAcc_SS(:,ii,jj,ss)] = fdr(pAcc_SS(:,ii,jj,ss),sigLevels(1));
            pD_SS(:,ii,jj,ss) = 2*min([mean(squeeze(realD(:,ii,jj,ss))<squeeze(nullD(:,ii,jj,ss,:)),2), ...
                mean(squeeze(realD(:,ii,jj,ss))>squeeze(nullD(:,ii,jj,ss,:)),2)],[],2);
            [~,isSigD_SS(:,ii,jj,ss)] = fdr(pD_SS(:,ii,jj,ss),sigLevels(1));
            
        end
   end
end  

% print out the p-values corresponding to Figure 5C
squeeze(pD_SS(vorder2plot,1,1,sets~=1 & sets~=3))'
