...
% irrelevant target over all trials
...
%what kind of anova?
typestr = 'matchTargIrrel_crossval';
...
for ss=1:nSubj
    ...
    for vv=1:length(VOIs)
        ...
        group = double((idmatch & trialData(vv).tasklabelsTBT(indsuse,:)==2) | (ormatch & trialData(vv).tasklabelsTBT(indsuse,:)==1));
